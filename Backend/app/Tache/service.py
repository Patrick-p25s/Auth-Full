...
class TaskService:
    ...
    async def create_task(self, new_task: TaskCreate) -> TaskOut:
        return await self.repo.create(
            {"tache": new_task.tache, "category": new_task.category}
        )

    async def update_task(self, id: int, task: TaskCreate) -> TaskOut:
        tache = await self.repo.get_by_id(id)
        if tache is None:
            raise HTTPException(
    ...
    async def delete_tache(self, id: int) -> bool:
        tache = await self.repo.get_by_id(id=id)
        if tache is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Tache non trouvé"
            )
    ...
    async def finish_toogle(self, id: int) -> TaskOut:
        tache = await self.repo.get_by_id(id=id)
        if tache is None:
            raise HTTPException(
    ...
    async def get_tache(self, id: int) -> TaskOut:
        tache = await self.repo.get_by_id(id=id)
        if tache is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Tache non trouvé"
